export const getListingData = async (id) => {
  try {
    let response = await fetch(
      "https://vrad-api.herokuapp.com/api/v1/listings/" + id
    );
...
export const getAreaListings = async (area) => {
  try {
    let response = await fetch(
      "https://vrad-api.herokuapp.com/api/v1/areas/" + area
    );
    let data = await response.json();
    let allListings = await Promise.all(
      data.listings.map(async (listing) => {
        let singleListing = await fetch(
          "https://vrad-api.herokuapp.com" + listing
        ).then((response) => response.json());
